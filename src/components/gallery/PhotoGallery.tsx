...
const PhotoGallery: React.FC<PhotoGalleryProps> = ({ images, categories, years }) => {
  ...
  const openLightbox = useCallback((image: GalleryImage) => {
    ...
    // Ouvrir le lightbox immédiatement sans délai
  ...
  const closeLightbox = useCallback(() => {
    ...
    // Délai court pour l'animation de fermeture
    setTimeout(() => {
      ...
      {filteredImages.length > 0 ? (
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 transition-opacity duration-300 ${
          ...
          {filteredImages.map((image, index) => (
            <div
              key={image.id}
              className="group relative overflow-hidden rounded-xl shadow-md cursor-pointer transform transition-all duration-500 hover:shadow-xl hover:-translate-y-1 hover:scale-[1.02] border border-gray-100"
            ...
            >
              <div className="relative h-64 overflow-hidden bg-gray-100">
                ...
                <div className="absolute inset-0 flex items-end p-4 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  <div className="w-full">
      ...
      {isLightboxOpen && selectedImage && (
        ...
        >
          ...
          {filteredImages.length > 1 && (
            <>
              <button
                className="absolute left-6 top-1/2 transform -translate-y-1/2 z-20 text-white bg-black/50 backdrop-blur-sm rounded-full p-3 hover:bg-black/70 transition-all duration-300 shadow-lg hover:scale-110"
                onClick={(e) => {
                  e.stopPropagation();
                  goToPrevImage();
                }}
              ...
              <button
                className="absolute right-6 top-1/2 transform -translate-y-1/2 z-20 text-white bg-black/50 backdrop-blur-sm rounded-full p-3 hover:bg-black/70 transition-all duration-300 shadow-lg hover:scale-110"
                onClick={(e) => {
                  e.stopPropagation();
                  goToNextImage();
                }}
          ...
          >
            ...
            <div className="relative bg-transparent rounded-lg overflow-hidden shadow-2xl">
              <img
                ...
                className="max-w-full max-h-[70vh] object-contain bg-transparent"
